...
プロジェクトを対象に実行した場合、そのプロジェクトの `DESIGN.md` に `## Unlearned Actions` セクションがあるか確認する。今回学習した `provider` / `action` に該当するエントリがあれば削除し、ドキュメント欠落が解消されたことを反映する（`GEMINI.md` の「レシピ実装ライフサイクル」参照）。
