...
書き込み先は **kit（submodule）内** のナレッジベース:
...
kit 内でコミット → workato-dev-kit に PR:
...
例外: `DESIGN.md` の Unlearned Actions 更新はワークスペースリポジトリの変更になるため、ワークスペースルートで `git add projects/<name>/DESIGN.md && git commit -m "..."` が必要。
