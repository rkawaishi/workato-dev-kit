...
**追記先**:
- レシピ構造全般 → `.cursor/rules/workato-recipe-format.mdc`
- Genie/MCP/Skill 構造 → `.cursor/rules/workato-agentic-format.mdc`
...
## DESIGN.md の Unlearned Actions 更新

プロジェクトを対象に実行した場合、そのプロジェクトの `DESIGN.md` に `## Unlearned Actions` セクションがあるか確認する。今回学習した `provider` / `action` に該当するエントリがあれば削除し、ドキュメント欠落が解消されたことを反映する（`@.claude/CLAUDE.md` の「レシピ実装ライフサイクル」参照）。

## Git 管理

書き込み先は基本的に **外側リポジトリ** `workato-dev-kit` 内:

- `docs/connectors/`, `docs/logic/`, `docs/platform/`, `docs/patterns/`, `docs/learned-patterns.md`
- `.cursor/rules/workato-recipe-format.mdc`, `.cursor/rules/workato-agentic-format.mdc`

外側でコミット:

```bash
git add docs/ .cursor/rules/
git commit -m "docs: learn from <project-name> recipes"
```

例外: `DESIGN.md` の Unlearned Actions 更新は内側 `projects/` リポジトリでの変更になるため、別途 `(cd projects/<name> && git add DESIGN.md && git commit -m "...")` が必要。詳細は `.cursor/rules/workato-multi-repo-git.mdc` 参照。
