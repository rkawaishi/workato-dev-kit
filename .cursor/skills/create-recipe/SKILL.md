...
詳細は `docs/platform/resource-providers.md` を参照。

#### 手順

1. **プロバイダーの特定**: Step 1 のヒアリングで決まったプロバイダーをリストアップ
2. **ツール検出**: 各プロバイダーに対して利用可能なツールを検出
   - MCP 系: `ToolSearch` で `mcp__<provider>__` を検索
   - CLI 系: `which gh`, `which workato` 等を実行
3. **リソース取得**: 検出できたツールでリソース情報を取得
   - 例: `gh repo list`, `mcp__jira__list_projects` 等
4. **選択肢の提示**: 取得した情報をユーザーに選択肢として提示
   ...
   どのプロジェクトに起票しますか？
   ```
5. **フォールバック**: ツールが見つからない、またはエラーの場合はサイレントにスキップし、Step 5 のヒアリングで従来通り確認する
