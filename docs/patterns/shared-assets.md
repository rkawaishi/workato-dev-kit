...
複数のプロジェクトで同じコネクション（例: Jira, Slack）や Recipe Function（例: マネージャー取得）を使い回したい。プロジェクトごとにコネクションや Function を複製すると、認証情報の管理やロジックの変更が煩雑になる。
...
| 状況 | 判断 |
|---|---|
| 2つ以上のプロジェクトで同じコネクション | Shared に移動 |
| 2つ以上のプロジェクトで同じロジック | Recipe Function として Shared に切り出し |
| 1つのプロジェクトでのみ使用 | プロジェクト内に保持（過度な共有化は避ける） |
| HRMS、通知、認証など横断的な機能 | 最初から Shared に作成 |
