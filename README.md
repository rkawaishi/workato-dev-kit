...
組織のレシピ・コネクタは `projects/` と `connectors/` に配置し、ワークスペースリポジトリで直接 git 管理します。
...
│   └── skills/                  # Cursor 用スキル（.claude/skills/ から自動生成）
├── scripts/
...
├── .claude/ → kit/.claude/      ← symlink (+ 組織独自ルール/スキル)
├── docs/ → kit/docs/            ← symlink
├── guides/ → kit/guides/        ← symlink
├── projects/                    ← 組織のレシピ（直接 git 管理）
└── connectors/                  ← 組織のカスタムコネクタ（直接 git 管理）
```
