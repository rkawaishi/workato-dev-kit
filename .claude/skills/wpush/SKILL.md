...
### 2. コネクション先行 push（新規コネクションがある場合）

新規コネクションがある場合、**コネクションだけを先に push** してユーザーに認証を依頼する。

手順:
1. レシピや他のアセットを一時的にプロジェクトディレクトリから退避
2. コネクションファイルのみでプロジェクトを push
3. ユーザーに認証設定を依頼:
...
4. ユーザーの確認後、退避したファイルを戻して全体を push

### 3. 通常 push

新規コネクションがない場合、または認証完了後:
