...
- `.claude/rules/workato-recipe-format.md`, `.claude/rules/workato-agentic-format.md`
...
例外: `DESIGN.md` の Unlearned Actions 更新は内側 `projects/` リポジトリでの変更になるため、別途 `(cd projects/<name> && git add DESIGN.md && git commit -m "...")` が必要。詳細は `@.claude/rules/workato-multi-repo-git.md` 参照。
