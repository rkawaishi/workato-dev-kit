...
1. **新規コネクションがある場合**: コネクション先行 push → UI で認証設定を依頼 → 確認後にレシピ push
2. **新規コネクションがない場合**: 通常 push
3. **push 後**: UI でのレシピ確認・フィールドマッピング確認を案内
4. **調整があった場合**: pull → `/learn-recipe` で学習
5. **テスト実行**: UI でテスト → エラーがあれば分析・修正
