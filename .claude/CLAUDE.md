...
- **未学習の記録**: ドキュメントに無いアクションをベストエフォートで実装した場合、そのプロジェクトの `DESIGN.md` の「未学習アクション」セクション、または当リポジトリの GitHub Issue に記録する（provider / action 名 / 判明した input/output があれば簡記）
...
- 学習が終わったら `DESIGN.md` の「未学習アクション」エントリから外す（または Issue をクローズする）
