...
|---|---|---|---|
| **内容** | スキル、ルール、ドキュメント | レシピ、ページ、DESIGN.md | カスタムコネクタ (connector.rb) |
| **性質** | フレームワーク（汎用） | 組織固有 | 組織固有 |
| **進化方法** | PR でスキル学習結果を反映 | 組織内で開発・コミット | 組織内で開発・コミット |
| **git 管理** | このリポジトリ | 組織の別リポジトリ (外側は gitignore) | 組織の別リポジトリ (外側は gitignore) |
