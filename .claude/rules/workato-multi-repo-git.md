...
(cd projects/<project-name> && git checkout -b feature/<branch>)
(cd projects/<project-name> && git add Recipes/ Connections/ DESIGN.md && git commit -m "<msg>")
(cd projects/<project-name> && git push origin feature/<branch>)
...
(cd projects/my-project && git checkout -b feature/<task>)
(cd projects/my-project && git add Recipes/ Connections/ && git commit -m "Add recipe: <name>")
...
# 内側
(cd projects/my-project && git branch --show-current)   # feature/foo
```
...
**`/wpush` と `git push` は別物。** `/wpush` は Workato API へのデプロイで、ローカル git 履歴には何もしない。ローカルの変更を git リモートにも残したい場合は、別途 `(cd projects/<name> && git push)` が必要。
