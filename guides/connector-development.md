...
API ヘルパーで push する（Platform CLI のプロファイルで認証、Ruby 不要）:
...
- Ruby 2.7.x 〜 3.1.x（ローカルテスト用）
- `gem install workato-connector-sdk`（ローカルテスト用）
- `settings.yaml` に認証情報を設定（git にコミットしない）
- Workato への push は `python3 scripts/workato-api.py sdk push` で可能（Ruby 不要）
