...
| **Cursor** | `.cursor/skills/`, `.cursor/rules/`（symlink） | `kit/framework/cursor/...`（`python3 scripts/sync_agents.py` で生成） |
...
| **エージェント中立** | `AGENTS.md` / `GEMINI.md`（symlink、同じ実体） | `kit/framework/AGENTS.md`（同上、CLAUDE.md + rules を集約） |
