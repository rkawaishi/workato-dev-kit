...
| エディタ | 利用者から見える場所 | 正本（kit 内の実体） |
|---|---|---|
| **Claude Code** | `.claude/skills/`, `.claude/rules/`（symlink） | `kit/framework/claude/skills/`, `kit/framework/claude/rules/` |
| **Cursor** | `.cursor/skills/`, `.cursor/rules/` | 正本から `bash scripts/sync-cursor-rules.sh` で生成 |

> **kit 開発者向け**: スキルやルールを変更するときは `framework/claude/` 配下を編集する。利用者の `.claude/` はそこへの symlink なので、kit のリリースが更新された瞬間に反映される。

どちらのエディタでも同じスキル (`/create-recipe`, `/wpush` 等) を同じ形式で呼び出せる。セットアップ手順はそれぞれのクイックスタートガイドを参照:
...
│   ├── skills/
│   └── hooks/
├── .cursor/                  # kit から symlink（Cursor 用）
...
kit リポジトリで `framework/claude/rules/` や `framework/claude/skills/` を変更した場合は、必ず同期スクリプトを実行する:

```bash
bash scripts/sync-cursor-rules.sh
```

コミット時は `framework/claude/` と `.cursor/` の両方の変更を含めること。
