...
| エディタ | スキル/ルールの場所 | 同期 |
|---|---|---|
| **Claude Code** | `.claude/skills/`, `.claude/rules/` | 正本（変更はここで行う） |
| **Cursor** | `.cursor/skills/`, `.cursor/rules/` | `bash scripts/sync-cursor-rules.sh` で `.claude/` から同期 |
...
`projects/` と `connectors/` はワークスペースリポジトリで直接 git 管理する。`kit/` は submodule として読み取り専用で参照する。
...
`.claude/rules/` や `.claude/skills/` を変更した場合は、必ず同期スクリプトを実行する:

```bash
bash scripts/sync-cursor-rules.sh
```

コミット時は `.claude/` と `.cursor/` の両方の変更を含めること。
